...
def draw_lines(img, lines, color=[255, 0, 0], thickness=6):
    for line in lines:
        for x1, y1, x2, y2 in line:
            ...
            top_horizon_line = ([0, img.shape[0]*0.5],
                                [img.shape[1], img.shape[0]*0.7])
            bottom_horizon_line = ([0, img.shape[0]], [
                                   img.shape[1], img.shape[0]])
            line_intersection_top = line_intersection(
                top_horizon_line, ([x1, y1], [x2, y2]))
            line_intersection_bottom = line_intersection(
                bottom_horizon_line, ([x1, y1], [x2, y2]))
            if line_intersection_top == None or line_intersection_bottom == None:
                return
            cv2.line(img, line_intersection_top,
                     line_intersection_bottom, color, thickness)
...
def pipeline(img):
    ...
    edge_image = canny(gaussian_blur_image,
    ...
    min_line_len = 3
    max_line_gap = 40
    hough_line_image = hough_lines(
        masked_edge_image, rho, theta, threshold, min_line_len, max_line_gap)
    sync_image = weighted_img(hough_line_image, image,
